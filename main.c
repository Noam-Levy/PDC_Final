...
int main(int argc, char *argv[]) {
  ...
  if (rank == MASTER) {
    ...
    if (!globalResults) {
      fprintf(stderr,
              "Faild allocating global results array. Terminating...\n");
      MPI_Abort(MPI_COMM_WORLD, __LINE__);
    }
  ...
  if (rank != MASTER) {
    ...
    if (!points) {
      fprintf(stderr,
              "Failed to allocate points array in process %d. Aborting...\n",
              rank);
      MPI_Abort(MPI_COMM_WORLD, __LINE__);
    }
  }
  ...
  // if applicable, allocate remainder tasks to master and offset slaves
  // accordingly
  if (rank == MASTER)
    chunk += remainder;
  else
  ...
  calculateTimes(localResults, startIndex, endIndex,
                 tCount); // calculate process' allocated times
  ...
  if (rank != MASTER) {
    ...
    for (i = 1; i < nProc; i++) {
      ...
      MPI_Recv(globalResults + offset, size, MPI_CRITERIA_T, i, TAG,

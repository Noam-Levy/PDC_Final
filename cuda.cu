 ...
 * @param lineNum interger represent the line number on which the error occured.
 ...
 **/
__device__ int isPointSatisfiesCriteria(Point *ref, Point *points, int size,
  ...
  for (int i = 0; i < size && count < minimumPoints; i++) {
    ...
    count +=
        dist >= 0 &&
        dist < maximumDistance; // negative distance indicates distance to self
  }
  return count >= minimumPoints;
}

/**
 * @brief Helper device function to atomically reset the global criteria met
 *counter.
 **/
...
/**
 * @brief Helper function to set the isCritetiraMet paramter to a referenced
 *result struct.
...
/**
 * @brief Checks which points satisfies the proximity criteria in a given time
 *t. results are saved in the referenced result struct.
 * @param points Array of points
 * @param size Size of points array
 * @param maximumDistance Maximum distance allowed between the reference point
 *to other points to satisfiy the criteria.
 ...
 **/
__global__ void checkProximityCriteria(Point *points, int size,
                                       double maximumDistance,
                                       int minimumPoints, criteria_t *result) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid > size || atomicAdd(&criteriaMetCounter, 0) >= MIN_CRITERIA_POINTS)
    return;

  if (isPointSatisfiesCriteria(&points[tid], points, size, maximumDistance,
                               minimumPoints)) {
    /*
      try to claim the right to increment criteriaMetCounter and update the
      results array. this is effectively a spin-lock for the right to update the
      results array.
    */
...
void computeProximities(Point *h_points, int size, criteria_t *h_results,
  ...
  // copy data to device
  err = cudaMemcpy(d_points, h_points, size * sizeof(Point),
                   cudaMemcpyHostToDevice);
  checkError(err, __LINE__ - 1);

  err = cudaMemcpy(d_results, h_results, chunk * sizeof(criteria_t),
                   cudaMemcpyHostToDevice);
  checkError(err, __LINE__ - 1);

  blocks = (int)floor((size * chunk) / THREADS_PER_BLOCK) +
           1; // +1 handles case where size * chunk < THREADS_PER_BLOCK

  for (i = 0; i < chunk; i++) {
    setPointsPositions<<<blocks, THREADS_PER_BLOCK>>>(d_points, size,
                                                      h_results[i].t);
    checkError(cudaGetLastError(), __LINE__ - 1);

    checkProximityCriteria<<<blocks, THREADS_PER_BLOCK>>>(
        d_points, size, maximumDistance, minimumPoints, &d_results[i]);
    checkError(cudaGetLastError(), __LINE__ - 1);

    setResultMetadata<<<1, 1>>>(&d_results[i]);
    checkError(cudaGetLastError(), __LINE__ - 1);
    cudaDeviceSynchronize(); // wait for all threads to finish with current
                             // timestamp
  }
  // copy results to host
  err = cudaMemcpy(h_results, d_results, chunk * sizeof(criteria_t),
